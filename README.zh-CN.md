...
4. 展开“成片精修”，选择极简、电影、动感或无标题开场；可粘贴每行一句的纯文本歌词生成本地时间初稿，也可导入带时间戳的 LRC，并选择舒缓、均衡或强烈节拍切换。时间初稿不是人声识别，需要在波形上检查。
5. 在有界波形时间轴中跳转、缩放，并用拖拽或键盘调整歌词边界与节拍点；手调切点只有点击明确的“重新生成”才会被覆盖，成片精修支持有界撤销/重做。
6. 在真实 `16:9`、`9:16` 或 `1:1` 舞台中随原始音乐预览。标题、歌词、节拍和画幅会自动保存在本机；刷新或异常退出后，重新选择同一音频即可恢复波形、预览和渲染，存储中不包含音频字节或文件路径。网页下载经过校验的创作方案；桌面 Creator 可选择“一键渲染视频”，通过原生保存窗口加入本机队列。
...
桌面版会先检查 FFmpeg 与 ffprobe。队列会显示等待、逐帧进度、完成、失败和取消状态，并支持失败重试和在 Finder 中定位成片。`.mp4` 默认生成 H.264，输出名为 `.mov` 时默认生成 ProRes。网页版导出创作方案而不是伪装成视频，音频也不会打包进 JSON。确定性视频导出使用完整 Canvas 渲染器，不包含额外叠加的 WebGPU 增强层。
...
- 成片精修支持四种开场标题、纯文本歌词本地时间初稿或带时间戳的 LRC 字幕、三档节拍切换和可拖拽波形时间轴；预览与编码视频共用同一个 Canvas 合成器。
...
视频编码目前要求目标 Mac 已安装 `ffmpeg` 与 `ffprobe`，也可在启动前设置 `FFMPEG_PATH` 和 `FFPROBE_PATH`；桌面版会在启用一键渲染前完成预检。打包后可运行 `npm run verify:packaged-render-worker -- "/path/to/Signal Field.app" --encode`，同时验证分派和真实 H.264 成片。签名与公证仍需要发布者自己的 Apple Developer 身份。若外置非 APFS 磁盘自动生成 `._app.asar` 并阻塞 electron-builder，请在 `/private/tmp` 等 APFS 工作副本构建，再复制成品。
...
app/lyric-timing.js         纯文本歌词的本地行级时间初稿（非人声识别）
app/production-spec.js      项目、标题、LRC、节拍和输出的严格创作方案
app/production-overlay.js   预览/导出共用的标题、歌词、强调和停顿合成器
app/timeline-editor-core.js 有界波形、坐标、歌词边界和节拍编辑核心
app/waveform-timeline.js    响应式 Canvas 波形与无障碍时间控制
